 ...
 *
 * It executes a {@link GestureDescription}: one or more strokes, each a {@link Path}
 * with a start delay and a duration, dispatched as a unit. It cannot stream - there
 * is no "keep this finger down and move it" call - which is why {@link TouchInjector}
 * buffers a drag and replays it on release. It also cannot send a bare key event
 * ({@code dispatchGesture}, not {@code performGlobalAction}).
 *
...
public class InjectionAccessibilityService extends AccessibilityService {
    ...
    public boolean inject(TouchInjector.Gesture gesture) {
        ...
        // StrokeDescription only needs *a* path, and a moveTo-only path is what every
        // tap-injection sample uses. It is also why the tap case above does not
        ...
        try {
            boolean accepted = dispatchGesture(description, callback, null);
            if (!accepted) {
    ...
    /** Result of the previous injection, mostly so a no-op can be explained. */
    private final GestureResultCallback callback = new GestureResultCallback() {
        @Override
        ...
        public void onCancelled(GestureDescription gestureDescription) {
            lastError = "gesture cancelled by the system";
