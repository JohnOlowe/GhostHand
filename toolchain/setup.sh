...
"$VENDOR/jre/bin/java" -version 2>&1 | head -1 | grep -q . || die "jre/bin/java does not run"
ok "$("$VENDOR/jre/bin/java" -version 2>&1 | head -1)  [JRE only - no javac, by design]"
...
"$VENDOR/jre/bin/java" -jar "$VENDOR/ecj.jar" -help 2>&1 | head -1 | grep -q "Eclipse Compiler" \
  || die "ecj.jar is not runnable"
ok "$("$VENDOR/jre/bin/java" -jar "$VENDOR/ecj.jar" -help 2>&1 | head -1)"
...
ok "$("$VENDOR/jre/bin/java" -cp "$VENDOR/d8.jar" com.android.tools.r8.D8 --version 2>&1 | head -1)"
"$VENDOR/jre/bin/java" -cp "$VENDOR/d8.jar" com.android.tools.r8.R8 --version 2>&1 | grep -q "^R8" \
  || die "R8 is not runnable"
ok "$("$VENDOR/jre/bin/java" -cp "$VENDOR/d8.jar" com.android.tools.r8.R8 --version 2>&1 | head -1)"
...
# ---------------------------------------------------------------------------
say "6b/8 patch android.jar so ECJ can compile lambdas"
# Stock android.jar files ship java.lang.invoke *except* LambdaMetafactory, and
# ECJ (unlike javac) refuses to compile a lambda without it. Add the missing
# class from the JRE's runtime image. Compile-time only: nothing from android.jar
# is ever packaged into the APK. See patch_android_jar.sh for the full argument.
if python3 -c "
import sys, zipfile
sys.exit(0 if 'java/lang/invoke/LambdaMetafactory.class'
         in zipfile.ZipFile(sys.argv[1]).namelist() else 1)" "$VENDOR/android.jar"; then
  ok "android.jar already has LambdaMetafactory"
else
  bash "$HERE/patch_android_jar.sh" "$VENDOR/android.jar" "$VENDOR/jre" \
    || die "could not patch android.jar (lambdas would fail to compile)"
fi

# ---------------------------------------------------------------------------
say "7/8 language-level classpath (android.jar minus the packages the JRE owns)"
